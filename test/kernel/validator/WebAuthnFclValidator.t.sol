...
import { WebAuthnFclVerifier } from "src/kernel/validator/webauthn/WebAuthnFclVerifier.sol";
import { P256VerifierWrapper } from "src/kernel/utils/P256VerifierWrapper.sol";
import { WebAuthnFclValidator } from "src/kernel/validator/webauthn/WebAuthnFclValidator.sol";
...
contract WebAuthnFclValidatorTest is KernelTestBase {
    ...
    function setUp() public {
        ...
        // Deploy a WebAuthnFclValidator contract using that RIP-7212 compliant P256Verifier
        // contract
    ...
    function getInitializeData() internal view override returns (bytes memory) {
        return abi.encodeWithSelector(
            ...
            webAuthNValidator,
            abi.encode("authenticator-id", x, y)
        );
    }
    ...
    function test_default_validator_enable() external override {
        UserOperation memory op = buildUserOperation(
            abi.encodeWithSelector(
                ...
                0,
                abi.encodeWithSelector(
                    webAuthNValidator.enable.selector, abi.encode("authenticator-id", x, y)
                ),
                Operation.Call
            )
        );
        ...
        (uint256 x2, uint256 y2) =
            WebAuthnFclValidator(address(webAuthNValidator)).getPublicKey(address(kernel));
    ...
    /// @dev Ensure that the validation won't revert when using the dummy signature bypass
    /// (challenge offset to uint256.max)
    function test_dontRevertForDummySig() public {
        ...
        bytes memory signature =
            abi.encode(true, authenticatorData, clientData, clientChallengeDataOffset, rs);

        // Check the sig (and ensure we didn't revert here)
        bool isValid = webAuthNTester.verifySignature(
            address(p256VerifierWrapper), bytes32(0), signature, x, y
        );
    ...
    function test_webAuthnSignatureGeneration(bytes32 _hash, uint256 _privateKey) public {
        ...
        bool isValid = webAuthNTester.verifySignature(
            address(p256VerifierWrapper), _hash, signature, pubX, pubY
        );
    ...
    function _generateWebAuthnSignature(
        uint256 _privateKey,
        bytes32 _hash
        ...
        )
    {
        ...
        // Prepare the client data (starting from a real webauthn challenge, then replacing only the
        // bytes needed for the challenge)
        bytes memory clientDataStart =
            hex"7b2274797065223a22776562617574686e2e676574222c226368616c6c656e6765223a22";
        ...
        WebAuthnFclVerifier.FclSignatureLayout memory sigLayout = WebAuthnFclVerifier
            .FclSignatureLayout({
...
/// @dev simple contract to format a webauthn challenge (using to convert stuff in memory during
/// test to calldata)
contract WebAuthNTester {
    function formatSigLayout(
        bytes32 _hash,
        WebAuthnFclVerifier.FclSignatureLayout calldata signatureLayout
    ...
    function verifySignature(
        ...
        return
            WebAuthnFclVerifier._verifyWebAuthNSignature(_p256Verifier, _hash, _signature, _x, _y);
    }
}
