...
contract ExecutionLogicTest is Test, FullDeploy {
    ...
    function test_executionLogic_onERC721Received_shouldReturnSelector() external {
        assertEq(
            ExecutionLogic.onERC721Received.selector,
            ExecutionLogic(vault).onERC721Received(address(0), address(0), 0, bytes(""))
        );
    }
    ...
    function test_executionLogic_execute_canExecuteArbitraryData() external {
        ...
        bytes memory returnData =
            IExecutionLogic(vault).execute(usdcAddress, 0, abi.encodeWithSignature("symbol()"));
        assertEq(abi.decode(returnData, (string)), IERC20Metadata(usdcAddress).symbol());

        returnData =
            IExecutionLogic(vault).execute(usdcAddress, 0, abi.encodeWithSignature("decimals()"));
        assertEq(abi.decode(returnData, (uint256)), IERC20Metadata(usdcAddress).decimals());

        returnData =
            IExecutionLogic(vault).execute(usdcAddress, 0, abi.encodeWithSignature("name()"));
        assertEq(abi.decode(returnData, (string)), IERC20Metadata(usdcAddress).name());
    ...
    function test_executionLogic_execute_executeWithNativeAmount() external {
        ...
        bytes memory returnData = IExecutionLogic(vault).execute(
            wmaticAddress, 0, abi.encodeWithSignature("balanceOf(address)", address(vault))
        );

        assertEq(abi.decode(returnData, (uint256)), 0);

        IExecutionLogic(vault).execute(wmaticAddress, 1 ether, abi.encodeWithSignature("deposit()"));
    ...
    function test_executionLogic_execute_shouldRevertIfTargetIsVaultAddress() external {
        vm.prank(vault);
        vm.expectRevert(IExecutionLogic.ExecutionLogic_ExecuteTargetCannotBeAddressThis.selector);
        IExecutionLogic(vault).execute(address(vault), 0, bytes(""));
    }

    function test_executionLogic_execute_shouldRevertIfExeternalCallReverted() external {
        vm.prank(vault);
        bytes memory data = abi.encodeWithSignature(
            "transferFrom(address,address,uint256)", address(this), address(vault), 1 ether
        );

        vm.expectRevert(
            abi.encodeWithSelector(
                IExecutionLogic.ExecutionLogic_ExecuteCallReverted.selector, usdcAddress, data
            )
        );
    ...
    function test_executionLogic_execute_accessControl() external {
        ...
        IExecutionLogic(vault).execute(
            usdcAddress, 0, abi.encodeWithSignature("balanceOf(address)", address(this))
        );
    ...
    function test_executionLogic_multicall() external {
        ...
        data[0] = abi.encodeCall(VaultLogic.depositERC20, (usdcAddress, 2500e6, vaultOwner));
        data[1] = abi.encodeCall(VaultLogic.depositERC20, (wethAddress, 1e18, vaultOwner));
        ...
        data[2] = abi.encodeCall(
            UniswapLogic.uniswapSwapExactInput, (tokens, poolFees, 1000e6, false, false, 0.1e18)
        );

        data[3] = abi.encodeCall(
            UniswapLogic.uniswapAddLiquidity, (nftId, 750e6, 0.4e18, false, true, 0.1e18)
        );
        ...
        data[5] = abi.encodeCall(
            IExecutionLogic.execute, (wmaticAddress, 1 ether, abi.encodeWithSignature("deposit()"))
        );
    ...
    function test_executionLogic_multicall_shouldRevertIfOneCallReverts() external {
        vm.startPrank(vaultOwner);
    ...
    function test_executionLogic_multicall_accessControl() external {
        ...
        vm.prank(executor);
        data[0] = abi.encodeCall(VaultLogic.withdrawNative, (executor, address(vault).balance));
    ...
    function test_executionLogic_taxedMulticall_shouldRevertIfOneCallReverts() external {
        vm.startPrank(vaultOwner);
    ...
    function test_executionLogic_taxedMulticall_instantFees_woTreasury() external {
        ...
        vm.prank(vaultOwner);
        bytes[] memory data = new bytes[](1);
        data[0] = abi.encodeCall(VaultLogic.withdrawNative, (vaultOwner, address(vault).balance));
    ...
    function test_executionLogic_taxedMulticall_instantFees_withTreasury() external {
        address treasury = makeAddr("treasury");
        ...
        vm.prank(vaultOwner);
        bytes[] memory data = new bytes[](1);
        data[0] = abi.encodeCall(VaultLogic.withdrawNative, (vaultOwner, 0.5e18));
