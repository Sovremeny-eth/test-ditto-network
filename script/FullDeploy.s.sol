...
import { ProtocolModuleList } from "../src/ditto/ProtocolModuleList.sol";

import { AccountAbstractionLogic } from "../src/ditto/vault/logics/AccountAbstractionLogic.sol";

import { VersionUpgradeLogic } from "../src/ditto/vault/logics/VersionUpgradeLogic.sol";
import { AccessControlLogic } from "../src/ditto/vault/logics/AccessControlLogic.sol";
import { EntryPointLogic } from "../src/ditto/vault/logics/EntryPointLogic.sol";
import { EntryPointLogicArbitrum } from
    "../src/ditto/vault/logics/arbitrumLogics/EntryPointLogicArbitrum.sol";
import { ExecutionLogic } from "../src/ditto/vault/logics/ExecutionLogic.sol";
import { ExecutionLogicArbitrum } from
    "../src/ditto/vault/logics/arbitrumLogics/ExecutionLogicArbitrum.sol";
import { VaultLogic } from "../src/ditto/vault/logics/VaultLogic.sol";
import { NativeWrapper } from "../src/ditto/vault/logics/OurLogic/helpers/NativeWrapper.sol";

import { UniswapLogic } from "../src/ditto/vault/logics/OurLogic/dexAutomation/UniswapLogic.sol";
import { PancakeswapLogic } from
    "../src/ditto/vault/logics/OurLogic/dexAutomation/PancakeswapLogic.sol";

import { DeltaNeutralStrategyLogic } from
    "../src/ditto/vault/logics/OurLogic/DeltaNeutralStrategyLogic.sol";
import { AaveActionLogic } from "../src/ditto/vault/logics/OurLogic/AaveActionLogic.sol";

import { AaveCheckerLogic } from "../src/ditto/vault/logics/Checkers/AaveCheckerLogic.sol";
import { DexCheckerLogicPancakeswap } from
    "../src/ditto/vault/logics/Checkers/DexCheckerLogicPancakeswap.sol";
import { DexCheckerLogicUniswap } from
    "../src/ditto/vault/logics/Checkers/DexCheckerLogicUniswap.sol";
import { PriceCheckerLogicPancakeswap } from
    "../src/ditto/vault/logics/Checkers/PriceCheckerLogicPancakeswap.sol";
import { PriceCheckerLogicUniswap } from
    "../src/ditto/vault/logics/Checkers/PriceCheckerLogicUniswap.sol";
import { PriceDifferenceCheckerLogicPancakeswap } from
    "../src/ditto/vault/logics/Checkers/PriceDifferenceCheckerLogicPancakeswap.sol";
import { PriceDifferenceCheckerLogicUniswap } from
    "../src/ditto/vault/logics/Checkers/PriceDifferenceCheckerLogicUniswap.sol";
import { TimeCheckerLogic } from "../src/ditto/vault/logics/Checkers/TimeCheckerLogic.sol";

import { StargateLogic } from "../src/ditto/vault/logics/OurLogic/bridges/StargateLogic.sol";
import { LayerZeroLogic } from "../src/ditto/vault/logics/OurLogic/bridges/LayerZeroLogic.sol";
import { CelerCircleBridgeLogic } from
    "../src/ditto/vault/logics/OurLogic/bridges/CelerCircleBridgeLogic.sol";

import { AaveLogicLens } from "../src/ditto/lens/AaveLogicLens.sol";
import { DexLogicLens } from "../src/ditto/lens/DexLogicLens.sol";

import { PartnerFeeModule } from "../src/ditto/vault/modules/PartnerFeeModule.sol";
...
contract FullDeploy is Script {
    ...
    function deployFactory() public returns (Registry.Contracts memory) {
        ...
        Registry.Contracts memory reg = Registry.contractsByChainId(block.chainid);

        if (address(reg.vaultFactoryProxyAdmin) == address(0)) {
            reg.vaultFactoryProxyAdmin = new ProxyAdmin(msg.sender); // FIXME
        }
        ...
        if (
            ...
            // vault factory
            VaultFactory _vaultFactory = new VaultFactory{ salt: prod ? saltProd : salt }(
                reg.logics.vaultUpgradeLogic,
                prod ? address(reg.vaultProxyAdminProd) : address(reg.vaultProxyAdmin)
            );
                ...
                reg.vaultFactoryProxyProd = ITransparentUpgradeableProxy(
                    address(
                        new TransparentUpgradeableProxy{ salt: saltProd }(
                            ...
                            abi.encodeCall(VaultFactory.initialize, (vm.addr(deployerPrivateKey)))
                        )
                    )
                );
            } else {
        ...
        if (prod) {
            if (address(reg.protocolFeesProd) == address(0)) {
                ...
                reg.protocolFeesProd =
                    new ProtocolFees{ salt: saltProd }(vm.addr(deployerPrivateKey));
            }

            if (address(reg.dittoBridgeReceiverProd) == address(0)) {
                reg.dittoBridgeReceiverProd = new DittoBridgeReceiver{ salt: saltProd }(
                    address(reg.vaultFactoryProxyProd), vm.addr(deployerPrivateKey)
                );

                reg.dittoBridgeReceiverProd.setBridgeContracts(
                    reg.stargateRouter, reg.layerZeroEndpoint
                );
            ...
            if (address(reg.protocolFees) == address(0)) {
                // dev protocol fees
                reg.protocolFees = new ProtocolFees{ salt: salt }(vm.addr(deployerPrivateKey));
            }

            if (address(reg.dittoBridgeReceiver) == address(0)) {
                reg.dittoBridgeReceiver = new DittoBridgeReceiver{ salt: salt }(
                    address(reg.vaultFactoryProxy), vm.addr(deployerPrivateKey)
                );
    ...
    function addImplementation(
        ...
        (bytes4[] memory selectors, address[] memory logicAddresses) = _getData(reg.logics);
        ...
        if (address(reg.vaultFactoryProxy) == address(0) || test) {
            VaultFactory _vaultFactory =
                new VaultFactory(reg.logics.vaultUpgradeLogic, address(reg.vaultProxyAdmin));
        ...
        vaultFactory =
            VaultFactory(address(prod ? reg.vaultFactoryProxyProd : reg.vaultFactoryProxy));
        ...
        vault = new Vault(
            selectors, logicAddresses, prod ? reg.protocolModuleListProd : reg.protocolModuleList
        );
    ...
    function deploySystemContracts(
        ...
        if (address(reg.lens.aaveLogicLens) == address(0)) {
            reg.lens.aaveLogicLens = new AaveLogicLens(reg.aaveAddressesProvider);
        }

        // common logic
        reg.logics.vaultUpgradeLogic = reg.logics.vaultUpgradeLogic == address(0) || test
            ? address(new UpgradeLogic())
            : reg.logics.vaultUpgradeLogic;

        reg.logics.accountAbstractionLogic = reg.logics.accountAbstractionLogic == address(0)
            || test
            ? address(new AccountAbstractionLogic(reg.entryPoint))
            : reg.logics.accountAbstractionLogic;

        if (prod) {
            console.log("vaultFactoryProxyProd", address(reg.vaultFactoryProxyProd));
            reg.logics.versionUpgradeLogicProd = reg.logics.versionUpgradeLogicProd == address(0)
                ? address(new VersionUpgradeLogic(VaultFactory(address(reg.vaultFactoryProxyProd))))
                : reg.logics.versionUpgradeLogicProd;
        } else {
            console.log("vaultFactoryProxy", address(reg.vaultFactoryProxy));
            reg.logics.versionUpgradeLogic = reg.logics.versionUpgradeLogic == address(0)
                ? address(new VersionUpgradeLogic(VaultFactory(address(reg.vaultFactoryProxy))))
                : reg.logics.versionUpgradeLogic;
        }

        reg.logics.accessControlLogic = reg.logics.accessControlLogic == address(0) || test
            ? address(new AccessControlLogic())
            : reg.logics.accessControlLogic;

        if (prod) {
            console.log("protocolFeesProd", address(reg.protocolFeesProd));
            reg.logics.entryPointLogicProd = reg.logics.entryPointLogicProd == address(0)
                ? block.chainid == 42161
                    ? address(new EntryPointLogicArbitrum(reg.automateGelato, reg.protocolFeesProd))
                    : address(new EntryPointLogic(reg.automateGelato, reg.protocolFeesProd))
                : reg.logics.entryPointLogicProd;

            reg.logics.executionLogicProd = reg.logics.executionLogicProd == address(0)
                ? block.chainid == 42161
            ...
            reg.logics.entryPointLogic = reg.logics.entryPointLogic == address(0) || test
                ? block.chainid == 42161
                    ? address(new EntryPointLogicArbitrum(reg.automateGelato, reg.protocolFees))
                    : address(new EntryPointLogic(reg.automateGelato, reg.protocolFees))
                : reg.logics.entryPointLogic;

            reg.logics.executionLogic = reg.logics.executionLogic == address(0) || test
                ? block.chainid == 42161
        ...
        reg.logics.pancakeswapLogic = reg.logics.pancakeswapLogic == address(0)
        ...
        // time checker
        reg.logics.timeCheckerLogic = reg.logics.timeCheckerLogic == address(0) || test
            ? address(new TimeCheckerLogic())
            : reg.logics.timeCheckerLogic;

        // price checkers
        reg.logics.priceCheckerLogicUniswap = reg.logics.priceCheckerLogicUniswap == address(0)
            || test
            ? address(new PriceCheckerLogicUniswap(reg.dittoOracle, address(reg.uniswapFactory)))
            : reg.logics.priceCheckerLogicUniswap;

        reg.logics.priceDifferenceCheckerLogicUniswap = reg
            .logics
            .priceDifferenceCheckerLogicUniswap == address(0) || test
            ? address(
                new PriceDifferenceCheckerLogicUniswap(reg.dittoOracle, address(reg.uniswapFactory))
            )
            : reg.logics.priceDifferenceCheckerLogicUniswap;

        reg.logics.priceCheckerLogicPancakeswap = reg.logics.priceCheckerLogicPancakeswap
            == address(0) || (test && reg.logics.priceCheckerLogicPancakeswap != address(1))
            ? address(
                new PriceCheckerLogicPancakeswap(reg.dittoOracle, address(reg.pancakeswapFactory))
            )
            : reg.logics.priceCheckerLogicPancakeswap;

        reg.logics.priceDifferenceCheckerLogicPancakeswap = reg
            .logics
            .priceDifferenceCheckerLogicPancakeswap == address(0)
            || (test && reg.logics.priceDifferenceCheckerLogicPancakeswap != address(1))
            ? address(
                new PriceDifferenceCheckerLogicPancakeswap(
                    reg.dittoOracle, address(reg.pancakeswapFactory)
                )
            )
            : reg.logics.priceDifferenceCheckerLogicPancakeswap;

        // aave + uni
        reg.logics.deltaNeutralStrategyLogic = reg.logics.deltaNeutralStrategyLogic == address(0)
        ...
        // aave
        reg.logics.aaveCheckerLogic = reg.logics.aaveCheckerLogic == address(0)
        ...
        reg.logics.aaveActionLogic = reg.logics.aaveActionLogic == address(0)
            ...
            ? address(new AaveActionLogic(reg.aaveAddressesProvider, reg.uniswapRouter))
            : reg.logics.aaveActionLogic;

        // bridges
        if (prod) {
            console.log("dittoBridgeReceiverProd", address(reg.dittoBridgeReceiverProd));
            reg.logics.stargateLogicProd = reg.logics.stargateLogicProd == address(0)
                ? address(new StargateLogic(reg.stargateRouter, address(reg.dittoBridgeReceiverProd)))
                : reg.logics.stargateLogicProd;

            reg.logics.layerZeroLogicProd = reg.logics.layerZeroLogicProd == address(0)
                ? address(
                    new LayerZeroLogic(reg.layerZeroEndpoint, address(reg.dittoBridgeReceiverProd))
                )
                : reg.logics.layerZeroLogicProd;
        } else {
            console.log("dittoBridgeReceiver", address(reg.dittoBridgeReceiver));
            reg.logics.stargateLogic = reg.logics.stargateLogic == address(0)
                ? address(new StargateLogic(reg.stargateRouter, address(reg.dittoBridgeReceiver)))
                : reg.logics.stargateLogic;

            reg.logics.layerZeroLogic = reg.logics.layerZeroLogic == address(0)
                ? address(new LayerZeroLogic(reg.layerZeroEndpoint, address(reg.dittoBridgeReceiver)))
                : reg.logics.layerZeroLogic;
        }

        reg.logics.celerCircleBridgeLogic = reg.logics.celerCircleBridgeLogic == address(0)
            ? address(new CelerCircleBridgeLogic(reg.celerCircleBridgeProxy, reg.usdc))
            : reg.logics.celerCircleBridgeLogic;

        // uniswap dex checker
        reg.logics.uniswapDexCheckerLogic = reg.logics.uniswapDexCheckerLogic == address(0) || test
            ? address(new DexCheckerLogicUniswap(reg.uniswapFactory, reg.uniswapNFTPositionManager))
            : reg.logics.uniswapDexCheckerLogic;

        reg.logics.pancakeswapDexCheckerLogic = reg.logics.pancakeswapDexCheckerLogic == address(0)
            || (test && reg.logics.pancakeswapDexCheckerLogic != address(1))
            ? address(
                new DexCheckerLogicPancakeswap(
                    reg.pancakeswapFactory, reg.pancakeswapNFTPositionManager
                )
            )
    ...
    function deployAndAddModules(
        ...
        if (prod) { } else {
            reg.partnerFeesContracts.partnerFees = (
                reg.partnerFeesContracts.partnerFees == address(0) || test
            ) ? address(new PartnerFee(initialOwner)) : reg.partnerFeesContracts.partnerFees;

            reg.modules.partnerFeeModule = (reg.modules.partnerFeeModule == address(0) || test)
                ? address(new PartnerFeeModule(reg.partnerFeesContracts.partnerFees))
            ...
            if (
                ...
                ProtocolModuleList(reg.protocolModuleList).addModule(
                    reg.modules.partnerFeeModule, selectors
                );
            }
        }
    ...
    function _getData(Registry.Logics memory logics)
        internal
        view
        returns (bytes4[] memory, address[] memory)
    {
        ...
        if (prod) {
            console2.log("versionUpgradeLogicProd", logics.versionUpgradeLogicProd);
        ...
        selectors[i++] = AccessControlLogic.setCrossChainLogicInactiveStatus.selector;
        ...
        selectors[i++] = UniswapLogic.uniswapWithdrawPositionByLiquidity.selector;
        ...
        if (logics.pancakeswapLogic != address(1)) {
            selectors[i++] = PancakeswapLogic.pancakeswapChangeTickRange.selector;
            ...
            selectors[i++] = PancakeswapLogic.pancakeswapSwapExactInput.selector;
            selectors[i++] = PancakeswapLogic.pancakeswapSwapExactOutputSingle.selector;
            selectors[i++] = PancakeswapLogic.pancakeswapSwapToTargetR.selector;
            selectors[i++] = PancakeswapLogic.pancakeswapWithdrawPositionByShares.selector;
            selectors[i++] = PancakeswapLogic.pancakeswapWithdrawPositionByLiquidity.selector;
        ...
        // price checker uni
        selectors[i++] = PriceCheckerLogicUniswap.priceCheckerUniswapInitialize.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapCheckGTTargetRate.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapCheckGTETargetRate.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapCheckLTTargetRate.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapCheckLTETargetRate.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapChangeTokensAndFeePriceChecker.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapChangeTargetRate.selector;
        selectors[i++] = PriceCheckerLogicUniswap.uniswapGetLocalPriceCheckerStorage.selector;
        console2.log("priceCheckerLogicUniswap", logics.priceCheckerLogicUniswap);
        for (uint256 k; k < 8; ++k) {
        ...
        selectors[i++] =
            PriceDifferenceCheckerLogicUniswap.priceDifferenceCheckerUniswapInitialize.selector;
        selectors[i++] = PriceDifferenceCheckerLogicUniswap.uniswapCheckPriceDifference.selector;
        selectors[i++] = PriceDifferenceCheckerLogicUniswap.uniswapCheckPriceDifferenceView.selector;
        selectors[i++] =
            PriceDifferenceCheckerLogicUniswap.uniswapChangeTokensAndFeePriceDiffChecker.selector;
        selectors[i++] =
            PriceDifferenceCheckerLogicUniswap.uniswapChangePercentageDeviationE3.selector;
        selectors[i++] =
            PriceDifferenceCheckerLogicUniswap.uniswapGetLocalPriceDifferenceCheckerStorage.selector;
        console2.log(
            "priceDifferenceCheckerLogicUniswap", logics.priceDifferenceCheckerLogicUniswap
        );
        for (uint256 k; k < 6; ++k) {
        ...
        if (logics.priceCheckerLogicPancakeswap != address(1)) {
            selectors[i++] = PriceCheckerLogicPancakeswap.priceCheckerPancakeswapInitialize.selector;
            selectors[i++] = PriceCheckerLogicPancakeswap.pancakeswapCheckGTTargetRate.selector;
            selectors[i++] = PriceCheckerLogicPancakeswap.pancakeswapCheckGTETargetRate.selector;
            selectors[i++] = PriceCheckerLogicPancakeswap.pancakeswapCheckLTTargetRate.selector;
            selectors[i++] = PriceCheckerLogicPancakeswap.pancakeswapCheckLTETargetRate.selector;
            selectors[i++] =
                PriceCheckerLogicPancakeswap.pancakeswapChangeTokensAndFeePriceChecker.selector;
            selectors[i++] = PriceCheckerLogicPancakeswap.pancakeswapChangeTargetRate.selector;
            selectors[i++] =
                PriceCheckerLogicPancakeswap.pancakeswapGetLocalPriceCheckerStorage.selector;
            console2.log("priceCheckerLogicPancakeswap", logics.priceCheckerLogicPancakeswap);
            for (uint256 k; k < 8; ++k) {
        ...
        if (logics.priceDifferenceCheckerLogicPancakeswap != address(1)) {
            ...
            selectors[i++] = PriceDifferenceCheckerLogicPancakeswap
                ...
                .selector;
            selectors[i++] =
                PriceDifferenceCheckerLogicPancakeswap.pancakeswapCheckPriceDifference.selector;
            selectors[i++] =
                PriceDifferenceCheckerLogicPancakeswap.pancakeswapCheckPriceDifferenceView.selector;
            selectors[i++] = PriceDifferenceCheckerLogicPancakeswap
            ...
            for (uint256 k; k < 6; ++k) {
                logicAddresses[j++] = logics.priceDifferenceCheckerLogicPancakeswap;
            }
        }
        ...
        if (logics.deltaNeutralStrategyLogic != address(1)) {
            ...
            selectors[i++] = DeltaNeutralStrategyLogic.initializeWithMint.selector;
            selectors[i++] = DeltaNeutralStrategyLogic.healthFactorsAndNft.selector;
            selectors[i++] = DeltaNeutralStrategyLogic.getTotalSupplyTokenBalance.selector;
            ...
            console2.log("deltaNeutralStrategyLogic", logics.deltaNeutralStrategyLogic);
            for (uint256 k; k < 10; ++k) {
        ...
        if (logics.aaveCheckerLogic != address(1)) {
            ...
            selectors[i++] = AaveCheckerLogic.getLocalAaveCheckerStorage.selector;
        ...
        if (logics.celerCircleBridgeLogic != address(1)) {
            // celer circle
            selectors[i++] = CelerCircleBridgeLogic.sendCelerCircleMessage.selector;
            selectors[i++] = CelerCircleBridgeLogic.sendBatchCelerCircleMessage.selector;
            console.log("celerCircleBridgeLogic", logics.celerCircleBridgeLogic);
            logicAddresses[j++] = logics.celerCircleBridgeLogic;
            logicAddresses[j++] = logics.celerCircleBridgeLogic;
        }

        // dex checker uniswap
        selectors[i++] = DexCheckerLogicUniswap.uniswapDexCheckerInitialize.selector;
        selectors[i++] = DexCheckerLogicUniswap.uniswapCheckOutOfTickRange.selector;
        selectors[i++] = DexCheckerLogicUniswap.uniswapCheckInTickRange.selector;
        selectors[i++] = DexCheckerLogicUniswap.uniswapCheckFeesExistence.selector;
        selectors[i++] = DexCheckerLogicUniswap.uniswapGetLocalDexCheckerStorage.selector;
        ...
        if (logics.pancakeswapDexCheckerLogic != address(1)) {
            ...
            selectors[i++] = DexCheckerLogicPancakeswap.pancakeswapDexCheckerInitialize.selector;
            selectors[i++] = DexCheckerLogicPancakeswap.pancakeswapCheckOutOfTickRange.selector;
            selectors[i++] = DexCheckerLogicPancakeswap.pancakeswapCheckInTickRange.selector;
            selectors[i++] = DexCheckerLogicPancakeswap.pancakeswapCheckFeesExistence.selector;
            selectors[i++] =
                DexCheckerLogicPancakeswap.pancakeswapGetLocalDexCheckerStorage.selector;
            console2.log("pancakeswapDexCheckerLogic", logics.pancakeswapDexCheckerLogic);
