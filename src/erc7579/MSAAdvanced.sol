...
contract MSAAdvanced is IMSA, ExecutionHelper, ModuleManager, HookManager {
        ...
        )
    {
        ...
        if (callType == CALLTYPE_BATCH) {
            ...
            // TODO: implement event emission for tryExecute singleCall
                ...
                if (!success) {
                    emit TryExecuteUnsuccessful(0, returnData[0]);
                }
            } else {
                revert UnsupportedExecType(execType);
            }
    ...
    function supportsModule(uint256 modulTypeId) external view virtual override returns (bool) {
        if (modulTypeId == MODULE_TYPE_VALIDATOR) {
            return true;
        } else if (modulTypeId == MODULE_TYPE_EXECUTOR) {
            return true;
        } else if (modulTypeId == MODULE_TYPE_FALLBACK) {
            return true;
        } else if (modulTypeId == MODULE_TYPE_HOOK) {
            return true;
        } else {
            return false;
        }
