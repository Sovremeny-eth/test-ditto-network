...
contract DittoBridgeReceiver is Ownable, IStargateReceiver, ILayerZeroReceiver {
    ...
    event DittoBridgeReceiverRevertData(address indexed vaultAddress, bytes payload, bytes reason);
    ...
    event LayerZeroWrongRecipient(address srcVaultAddress, address dstVaultAddress);
    ...
    function withdrawToken(address token) external onlyOwner {
        if (token == address(0)) {
            ...
            TransferHelper.safeTransfer(
                token, msg.sender, TransferHelper.safeGetBalance(token, address(this))
            );
        }
    }
    ...
    function sgReceive(
        ...
        address dstChainVaultAddress = vaultFactory.predictDeterministicVaultAddress(
            stargatePayload.srcChainVaultOwner, stargatePayload.srcChainVaultId
        );
        ...
        if (srcChainVaultAddress == dstChainVaultAddress) {
            ...
            (bool success, bytes memory revertReason) = dstChainVaultAddress.call(callData);

            // if revert -> emit the reason and stop tx execution
            if (!success) {
                emit DittoBridgeReceiverRevertData(
                    dstChainVaultAddress, stargatePayload.payload, revertReason
                );
            }
        } else {
            ...
            TransferHelper.safeTransfer(token, stargatePayload.srcChainVaultOwner, amountLD);
        }
    }
    ...
    function lzReceive(
        ...
        // gets the vault address for the destination chain
        address dstChainVaultAddress = vaultFactory.predictDeterministicVaultAddress(
            layerZeroPayload.srcChainVaultOwner, layerZeroPayload.srcChainVaultId
        );
        ...
        if (srcChainVaultAddress == dstChainVaultAddress) {
            ...
            (bool success, bytes memory revertReason) =
                dstChainVaultAddress.call(layerZeroPayload.payload);

            // if revert -> emit the reason and stop tx execution
            if (!success) {
                emit DittoBridgeReceiverRevertData(
                    dstChainVaultAddress, layerZeroPayload.payload, revertReason
                );
            }
        } else {
            // if addresses are different, we emit event
            emit LayerZeroWrongRecipient(srcChainVaultAddress, dstChainVaultAddress);
        }
    }
    ...
    /// @dev Validates that the caller is the expected bridge contract and decodes the source chain
    /// vault address.
    ...
    function _validateSender(
        ...
        assembly ("memory-safe") {
            srcChainVaultAddress := shr(96, mload(add(_srcChainVaultAddress, 32)))
        }
    }
    ...
    function _decodePayload(bytes calldata payload)
        internal
        pure
        returns (BridgePayload calldata bridgePayload)
    {
        assembly ("memory-safe") {
