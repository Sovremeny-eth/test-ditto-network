...
contract VaultFactory is IVaultFactory, Ownable {
    ...
    /// @dev If vaultFactory is already initialized - throws `VaultFactory_AlreadyInitialized`
    /// error.
    ...
    /// @inheritdoc IVaultFactory
    function setBridgeReceiverContract(address _dittoBridgeReceiver) external onlyOwner {
        dittoBridgeReceiver = _dittoBridgeReceiver;
    }

    /// @inheritdoc IVaultFactory
    function setEntryPointCreatorAddress(address _entryPointCreator) external onlyOwner {
    ...
    /// @inheritdoc IVaultFactory
    function implementation(uint256 version) external view returns (address impl_) {
    ...
    function _deploy(address creator, uint256 version, uint16 vaultId) private returns (address) {
        if (vaultId == 0) {
        ...
        // The following section of code is used to create a new contract instance using create2
        // opcode.
        ...
        assembly ("memory-safe") {
            ...
            vault := create2(0, add(vaultInitcode, 32), mload(vaultInitcode), keccak256(0, 64))
        }
        ...
        IVaultProxyAdmin(vaultProxyAdmin).initializeImplementation(
            vault, _implementations[version - 1]
        );
    ...
    function _validateVersion(uint256 version) private view returns (uint256 latestVersion) {
    ...
    function _getVaultInitcode() private view returns (bytes memory bytecode) {
        ...
        // runtime code:
        //   validate caller:
        //     1. If the caller is anyone other than `proxyAdmin` -> delegates call
        // to`vaultImplementation`.
        //     2. If the caller is the `proxyAdmin` -> delegates call to `upgradeLogic`.
        return abi.encodePacked(
            hex"6000605080600b833990f3600036818037808036813373",
            vaultProxyAdmin,
            hex"14602a578019546040565b73",
            upgradeLogic,
            hex"5b5af43d82803e3d8282604e57fd5bf3"
        );
    }
}
