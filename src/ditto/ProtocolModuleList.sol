...
/// @dev Manages the list of modules added to the protocol, their activation status, and associated
/// selectors.
contract ProtocolModuleList is Ownable, IProtocolModuleList {
    ...
    function addModule(address moduleAddress, bytes4[] calldata selectors) external onlyOwner {
    ...
    /// @inheritdoc IProtocolModuleList
    function listedModule(address moduleAddress) external view returns (bool listed) {
        return _listedModule(moduleAddress);
    }

    /// @inheritdoc IProtocolModuleList
    function isModuleInactive(address moduleAddress) external view returns (bool inactive) {
        return _moduleList[moduleAddress].inactive;
    }

    /// @inheritdoc IProtocolModuleList
    function getSelectorsByModule(address moduleAddress)
        external
        view
        returns (bytes memory selectors, bool inactive)
    {
    ...
    function _listedModule(address moduleAddress) private view returns (bool listed) {
        assembly ("memory-safe") {
