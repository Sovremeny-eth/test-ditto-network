...
abstract contract PriceCheckerLogicBase is IPriceCheckerLogicBase {
    ...
    function _getStorageUnsafe(bytes32 pointer)
        internal
        pure
        returns (PriceCheckerStorage storage s)
    {
    ...
    /// @param pointer Pointer to the strategy's storage location.
    /// @return s The storage slot for PriceCheckerStorage structure.
    function _getStorage(bytes32 pointer) internal view returns (PriceCheckerStorage storage s) {
    ...
    function _changeTokensAndFeePriceChecker(IUniswapV3Pool dexPool, bytes32 pointer) internal {
        PriceCheckerStorage storage s = _getStorage(pointer);
    ...
    function _currentRate(PriceCheckerStorage storage s) private view returns (uint256) {
    ...
    /// @param s The storage slot for PriceCheckerStorage structure.
    function _changeTokensAndFee(IUniswapV3Pool dexPool, PriceCheckerStorage storage s) private {
