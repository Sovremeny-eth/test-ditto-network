...
import { IPriceDifferenceCheckerLogicBase } from
    "../../interfaces/checkers/IPriceDifferenceCheckerLogicBase.sol";

/// @title PriceDifferenceCheckerLogicBase
abstract contract PriceDifferenceCheckerLogicBase is IPriceDifferenceCheckerLogicBase {
    // =========================
    // Constructor
    // =========================
    ...
    function _getStorageUnsafe(bytes32 pointer)
        internal
        pure
        returns (PriceDifferenceCheckerStorage storage s)
    {
    ...
    /// @notice Initializes the PriceDifferenceChecker contract by setting the token addresses and
    /// percentage of difference.
    /// @param dexPool The Uniswap V3 pool from which to check the price.
    /// @param percentageDeviation_E3 The percentage of difference allowed between the two token
    /// prices.
    ...
    /// @notice Checks the percentage difference between the current price and the last checked
    /// price.
    ...
    function _checkPriceDifference(bytes32 pointer) internal returns (bool success) {
    ...
    /// @param pointer The bytes32 pointer value.
    /// @return success True if the percentage difference is within an acceptable range.
    function _checkPriceDifferenceView(bytes32 pointer) public view returns (bool success) {
        PriceDifferenceCheckerStorage storage s = _getStorage(pointer);
    ...
    function _getLocalPriceDifferenceCheckerStorage(bytes32 pointer)
        internal
        pure
        returns (PriceDifferenceCheckerStorage memory priceDifferenceCheckerStorage)
    {
    ...
    function _checkPriceDifference(PriceDifferenceCheckerStorage storage s)
        private
        view
        returns (bool success, uint256 currentPrice)
    {
        ...
        if (percentageDeviation_E3 > E3) {
            unchecked {
                success = currentPrice > (s.lastCheckPrice * (percentageDeviation_E3)) / E3;
            }
        } else {
            unchecked {
                success = currentPrice < (s.lastCheckPrice * (percentageDeviation_E3)) / E3;
    ...
    function _setPercentageDeviation(
        ...
        emit PriceDifferenceCheckerSetNewDeviationThreshold(percentageDeviation_E3);
    }
