...
contract TimeCheckerLogic is ITimeCheckerLogic, BaseContract {
    ...
    function _getStorageUnsafe(bytes32 pointer)
        internal
        pure
        returns (TimeCheckerStorage storage s)
    {
    ...
    /// @param pointer Pointer to the strategy's storage location.
    /// @return s The storage slot for TimeCheckerStorage structure.
    function _getStorage(bytes32 pointer) internal view returns (TimeCheckerStorage storage s) {
    ...
    /// @inheritdoc ITimeCheckerLogic
    function checkTime(bytes32 pointer) external onlyVaultItself returns (bool) {
        TimeCheckerStorage storage s = _getStorage(pointer);

        bool enoughTimePassed = _enoughTimePassed(s.lastActionTime, s.timePeriod);
    ...
    /// @inheritdoc ITimeCheckerLogic
    function setTimePeriod(uint64 timePeriod, bytes32 pointer) external onlyOwnerOrVaultItself {
    ...
    /// @inheritdoc ITimeCheckerLogic
    function getLocalTimeCheckerStorage(bytes32 pointer)
        external
        view
    ...
    function _enoughTimePassed(uint256 startTime, uint256 period) internal view returns (bool) {
