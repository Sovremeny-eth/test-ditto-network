...
contract DexCheckerLogicBase is IDexCheckerLogicBase, BaseContract {
    ...
    constructor(IUniswapV3Factory _dexFactory, INonfungiblePositionManager _dexNftPositionManager) {
    ...
    function _getStorageUnsafe(bytes32 pointer)
        internal
        pure
        returns (DexCheckerStorage storage s)
    {
    ...
    /// @return s The storage slot for DexCheckerStorage structure.
    function _getStorage(bytes32 pointer) internal view returns (DexCheckerStorage storage s) {
    ...
    /// @dev This function sets the initialization flag and assigns the provided NFT ID to the
    /// storage.
    ...
    function _dexCheckerInitialize(uint256 nftId, bytes32 pointer) internal onlyVaultItself {
    ...
    function _getTickRangeAndCurrentTick(bytes32 pointer)
        internal
        view
        returns (int24, int24, int24)
    {
        ...
        (address token0, address token1, uint24 poolFee, int24 tickLower, int24 tickUpper,) =
            DexLogicLib.getNftData(s.nftId, dexNftPositionManager);

        IUniswapV3Pool dexPool = DexLogicLib.dexPool(token0, token1, poolFee, dexFactory);
    ...
    /// @dev This function retrieves the NFT data using the DexLogicLib and checks if there are any
    /// fees
    /// accumulated for the NFT position in the Uniswap V3 pool.
    /// @param pointer The pointer referencing the DexChecker's storage.
    /// @return Returns true if either of the fees (amount0 or amount1) is greater than 0, otherwise
    /// returns false.
    function _checkFeesExistence(bytes32 pointer) internal view returns (bool) {
        ...
        (uint256 amount0, uint256 amount1) =
            PositionValueMod.fees(dexNftPositionManager, nftId, dexPool);
