...
contract AaveCheckerLogic is IAaveCheckerLogic, BaseContract {
    ...
    function _getStorageUnsafe(bytes32 pointer)
        internal
        pure
        returns (AaveCheckerStorage storage s)
    {
    ...
    /// @param pointer Pointer to the strategy's storage location.
    /// @return s The storage slot for AaveCheckerStorage structure.
    function _getStorage(bytes32 pointer) internal view returns (AaveCheckerStorage storage s) {
    ...
    function checkHF(bytes32 pointer) external view returns (bool) {
        ...
        return (healthFactor <= s.lowerHFBoundary || healthFactor > s.upperHFBoundary);
    }
    ...
    /// @inheritdoc IAaveCheckerLogic
    function getHFBoundaries(bytes32 pointer)
        external
        view
        returns (uint256 lowerHFBoundary, uint256 upperHFBoundary)
    {
        AaveCheckerStorage storage s = _getStorage(pointer);
    ...
    /// @inheritdoc IAaveCheckerLogic
    function getLocalAaveCheckerStorage(bytes32 pointer)
        external
        view
        returns (uint256 lowerHFBoundary, uint256 upperHFBoundary, address user, bool initialized)
    {
        AaveCheckerStorage storage s = _getStorageUnsafe(pointer);
