...
import {
    ...
    IGelato,
    IProxyModule
} from "@gelato/contracts/integrations/Types.sol";
...
contract EntryPointLogic is IEntryPointLogic, BaseContract, DittoFeeBase {
    ...
    constructor(IAutomate automate, IProtocolFees protocolFees) DittoFeeBase(protocolFees) {
    ...
    function _getLocalStorage() internal view returns (EntryPointStorage storage eps) {
    ...
    function activateVault(bytes[] calldata callbacks) external onlyOwnerOrVaultItself {
    ...
    /// @inheritdoc IEntryPointLogic
    function deactivateVault(bytes[] calldata callbacks) external onlyOwnerOrVaultItself {
        EntryPointStorage storage eps = _getLocalStorage();
        if (eps.inactive) {
    ...
    /// @inheritdoc IEntryPointLogic
    function activateWorkflow(uint256 workflowKey) external onlyOwnerOrVaultItself {
        EntryPointStorage storage eps = _getLocalStorage();
    ...
    /// @inheritdoc IEntryPointLogic
    function deactivateWorkflow(uint256 workflowKey) external onlyOwnerOrVaultItself {
        EntryPointStorage storage eps = _getLocalStorage();
        _verifyWorkflowKey(eps, workflowKey);
    ...
    /// @inheritdoc IEntryPointLogic
    function getWorkflow(uint256 workflowKey) external view returns (Workflow memory) {
    ...
    /// @inheritdoc IEntryPointLogic
    function canExecWorkflowCheck(uint256 workflowKey) external view returns (bool, bytes memory) {
            ...
            if (success) {
                ...
                if (!success) {
                    return (
                        false,
                        abi.encodePacked(
                            "Workflow cannot be executed: ", Strings.toString(workflowKey)
                            )
                    );
                }
            }
    ...
    /// @inheritdoc IEntryPointLogic
    function run(uint256 workflowKey) external virtual onlyRoleOrOwner(Constants.EXECUTOR_ROLE) {
    ...
    /// @inheritdoc IEntryPointLogic
    function dedicatedMessageSender() public view returns (address dedicatedMsgSender) {
        (dedicatedMsgSender,) = _opsProxyFactory.getProxyOf(address(this));
    }

    /// @inheritdoc IEntryPointLogic
    function createTask(uint256 workflowKey) external payable onlyVaultItself returns (bytes32) {
        EntryPointStorage storage eps = _getLocalStorage();
    ...
    function _verifyWorkflowKey(EntryPointStorage storage eps, uint256 workflowKey) private view {
    ...
    function _createTask(
        ...
        moduleData.args[0] = _resolverModuleArg(
            address(this), abi.encodeCall(this.canExecWorkflowCheck, (workflowKey))
        );

        bytes memory execData = abi.encodeCall(this.runGelato, (workflowKey));

        bytes32 taskId = _automate.createTask(address(this), execData, moduleData, Constants.ETH);
    ...
    /// @dev This function deletes the task from storage and then calls the automation logic to
    /// cancel the task.
    /// @param workflowKey Unique identifier for the associated workflow.
    /// @param eps Reference to EntryPointStorage where the task is stored.
    function _cancelTask(uint256 workflowKey, EntryPointStorage storage eps) private {
    ...
    function _deactivateWorkflow(uint256 workflowKey, Workflow storage workflow) private {
