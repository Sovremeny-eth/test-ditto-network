...
contract DexBaseLogic is IDexBaseLogic {
    ...
    function _changeTickRange(
...

        {
            ...
            (token0, token1, poolFee, tickLower, tickUpper, liquidity, dexPool) = _getData(nftId);
            ...
            (token0Amount, token1Amount) =
                DexLogicLib.withdrawPositionMEVUnsafe(nftId, liquidity, dexNftPositionManager);
        ...
        return DexLogicLib.mintNftMEVUnsafe(
            token0Amount,
            token1Amount,
            newLowerTick,
            newUpperTick,
    ...
    function _addLiquidity(
        ...
        // To avoid passing token addresses to the _increaseLiquidity method, the approve is done
        // here
        ...
        DexLogicLib.increaseLiquidityMEVUnsafe(
            nftId, token0Amount, token1Amount, dexNftPositionManager
        );
    }
    ...
    /// @param deviationThresholdE18 Maximum allowed spotPrice deviation from the oracle price.
    function _autoCompound(uint256 nftId, uint256 deviationThresholdE18) internal {
        ...
        (amount0, amount1) = DexLogicLib.swapToTargetRMEVUnsafe(
            tickLower, tickUpper, amount0, amount1, dexPool, token0, token1, poolFee, dexRouter
        );

        // To avoid passing token addresses to the _increaseLiquidity method, the approve is done
        // here
        TransferHelper.safeApprove(token0, address(dexNftPositionManager), amount0);
        TransferHelper.safeApprove(token1, address(dexNftPositionManager), amount1);

        // adds liquidity to the NFT
        DexLogicLib.increaseLiquidityMEVUnsafe(nftId, amount0, amount1, dexNftPositionManager);
    }
    ...
    function _swapExactInput(
        ...
        for (uint256 i; i < poolFees.length;) {
            ...
            IUniswapV3Pool dexPool = DexLogicLib.dexPool(tokenIn, tokenOut, poolFee, dexFactory);
            DexLogicLib.MEVCheck(deviationThresholdE18, dexPool, PERIOD);

            amountIn =
                DexLogicLib.swapExactInputMEVUnsafe(tokenIn, tokenOut, poolFee, amountIn, dexRouter);
        }
    ...
    function _swapExactOutputSingle(
        ...
        uint256 tokenInBalance = TransferHelper.safeGetBalance(tokenIn, address(this));
        TransferHelper.safeApprove(tokenIn, address(dexRouter), tokenInBalance);

        amountIn =
            DexLogicLib.swapExactOutputMEVUnsafe(tokenIn, tokenOut, poolFee, amountOut, dexRouter);
    ...
    function _swapToTargetR(
        ...
        return DexLogicLib.swapAssetsMEVUnsafe(
            token0Amount,
            token1Amount,
            amount1Target,
            targetRE18,
    ...
    function _withdrawPositionByShares(
        ...
        uint128 liquidity = DexLogicLib.getLiquidity(nftId, dexNftPositionManager);
    ...
    function _withdrawPositionByLiquidity(
        ...
        uint128 totalLiquidity = DexLogicLib.getLiquidity(nftId, dexNftPositionManager);
    ...
    /// @param nftId The ID of the NFT representing the position.
    function _collectFees(uint256 nftId) internal returns (uint256 amount0, uint256 amount1) {
        (amount0, amount1) = DexLogicLib.collectFees(nftId, dexNftPositionManager);
    ...
    function _getData(uint256 nftId)
        private
        view
        returns (
            address token0,
