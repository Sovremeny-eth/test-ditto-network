...
import { IDeltaNeutralStrategyLogic } from
    "../../interfaces/ourLogic/IDeltaNeutralStrategyLogic.sol";
...
contract DeltaNeutralStrategyLogic is IDeltaNeutralStrategyLogic, BaseContract {
    ...
    bytes32 private immutable DNS_COMMON_STORAGE = keccak256("vault.dns.common.storage");
    ...
    function _getStorageUnsafe(bytes32 pointer)
        internal
        pure
        returns (DeltaNeutralStrategyStorage storage s)
    {
        assembly ("memory-safe") {
            s.slot := pointer
        }
    ...
    function _getStorage(bytes32 pointer)
        internal
        view
        returns (DeltaNeutralStrategyStorage storage s)
    {
    ...
    function initialize(
        ...
        // set storage
        s.supplyTokenAave = IAToken(AaveLogicLib.aSupplyTokenAddress(supplyToken, aavePool));
        s.debtTokenAave = IVariableDebtToken(AaveLogicLib.aDebtTokenAddress(debtToken, aavePool));
    ...
    function initializeWithMint(
        ...
        // set storage
        s.supplyTokenAave = IAToken(AaveLogicLib.aSupplyTokenAddress(p.supplyToken, aavePool));
        s.debtTokenAave = IVariableDebtToken(AaveLogicLib.aDebtTokenAddress(p.debtToken, aavePool));
    ...
    /// @inheritdoc IDeltaNeutralStrategyLogic
    function healthFactorsAndNft(bytes32 pointer)
        external
        view
        returns (uint256 targetHF, uint256 currentHF, uint256 uniswapV3NftId)
    {
        ...
        currentHF = AaveLogicLib.getCurrentHF(address(this), aavePoolAddressesProvider);
        uniswapV3NftId = s.uniswapV3NftId;
    }

    /// @inheritdoc IDeltaNeutralStrategyLogic
    function getTotalSupplyTokenBalance(bytes32 pointer) external view returns (uint256) {
        DeltaNeutralStrategyStorage storage s = _getStorageUnsafe(pointer);
        ...
        (,, uint24 poolFee,,,) = DexLogicLib.getNftData(nftId, dexNftPositionManager);

        IUniswapV3Pool dexPool = DexLogicLib.dexPool(supplyToken, debtToken, poolFee, dexFactory);

        return _getTotalSupplyTokenBalance(
            0, nftId, supplyTokenAave, supplyToken, debtTokenAave, debtToken, dexPool
        );
    }
    ...
    /// @inheritdoc IDeltaNeutralStrategyLogic
    function setNewTargetHF(uint256 newTargetHF, bytes32 pointer) external onlyOwnerOrVaultItself {
    ...
    function setNewNftId(
        ...
        _validateTokens(newNftId, supplyToken, s.debtTokenAave.UNDERLYING_ASSET_ADDRESS());
    ...
    function withdraw(
        ...
        _strategyActions(
            0, shareE18, deviationThresholdE18, s.uniswapV3NftId, supplyTokenAave, supplyToken, s
        );
        emit DeltaNeutralStrategyWithdraw();
    }

    /// @inheritdoc IDeltaNeutralStrategyLogic
    function rebalance(uint256 deviationThresholdE18, bytes32 pointer) external onlyVaultItself {
        DeltaNeutralStrategyStorage storage s = _getStorage(pointer);
    ...
    function _rebalance(
        ...
        _strategyActions(
            0, 0, deviationThresholdE18, s.uniswapV3NftId, supplyTokenAave, supplyToken, s
        );
    ...
    function _setNewTargetHF(
        ...
        (uint256 lowerHFBoundary, uint256 upperHFBoundary) =
            IAaveCheckerLogic(address(this)).getHFBoundaries(pointerToAaveChecker);
    ...
    function _strategyActions(
...

        {
            (,, uint24 poolFee,,,) = DexLogicLib.getNftData(nftId, dexNftPositionManager);
            IUniswapV3Pool dexPool =
                DexLogicLib.dexPool(supplyToken, debtToken, poolFee, dexFactory);
        ...
        if (shareE18 > 0) {
            ...
            unchecked {
                ...
                totalToken0Balance = (totalToken0Balance * (E18 - shareE18)) / E18;
            }
        }

        // get target amounts for new value
        (uint256 amountToSupply, uint256 amountToBorrow) = _getAmountToSupplyAndToBorrow(
        ...
        uint256 supplyBalanceBefore = TransferHelper.safeGetBalance(supplyToken, address(this));
        uint256 debtBalanceBefore = TransferHelper.safeGetBalance(debtToken, address(this));
        ...
        uint256 debtBalanceAfter = TransferHelper.safeGetBalance(debtToken, address(this));
        ...
        if (shareE18 > 0) {
            ...
            uint256 supplyBalanceAfter = TransferHelper.safeGetBalance(supplyToken, address(this));
            ...
            if (supplyBalanceBefore >= supplyBalanceAfter) {
                unchecked {
                    supplyAmountToUni =
                        aaveActionCache.amountToDeposit - (supplyBalanceBefore - supplyBalanceAfter);
                }
            } else {
                unchecked {
                    supplyAmountToUni = supplyBalanceAfter - supplyBalanceBefore;
                }
            }
            if (debtBalanceAfter > debtBalanceBefore) {
                unchecked {
    ...
    function _bringToTheAmounts(
        ...
        uint256 supplyTokenAaveBalance =
            TransferHelper.safeGetBalance(address(supplyTokenAave), address(this));

        uint256 debtTokenAaveBalance =
            TransferHelper.safeGetBalance(address(debtTokenAave), address(this));

        if (amountToSupply >= supplyTokenAaveBalance && amountToBorrow >= debtTokenAaveBalance) {
    ...
    function _bringingToTargetSupplyAmount(
        ...
        if (amountToSupply > supplyTokenAaveBalance) {
            ...
            (,, dataCache.poolFee, dataCache.tickLower, dataCache.tickUpper, dataCache.nftLiquidity)
            ...
            if (amountToDeposit < deltaToSupply) {
                if (dataCache.nftLiquidity != 0) {
                    ...
                    if (supplyToken > debtToken) {
                        liquidityToWithdraw = LiquidityAmounts.getLiquidityForAmount1(
                            sqrtPriceX96,
                            TickMath.getSqrtRatioAtTick(dataCache.tickLower),
                            deltaToSupply - amountToDeposit
                        );
                    } else {
                        liquidityToWithdraw = LiquidityAmounts.getLiquidityForAmount0(
                            sqrtPriceX96,
                            TickMath.getSqrtRatioAtTick(dataCache.tickUpper),
                            deltaToSupply - amountToDeposit
                        );
                    }
                    ...
                    (uint256 supplyWithdrawed, uint256 debtWithdrawed) = DexLogicLib
                        .withdrawPositionMEVUnsafe(nftId, liquidityToWithdraw, dexNftPositionManager);

                    if (supplyToken > debtToken) {
                        (supplyWithdrawed, debtWithdrawed) = (debtWithdrawed, supplyWithdrawed);
                    }
                    unchecked {
                    ...
                    if (deltaToSupply > amountToDeposit && debtWithdrawed > 0) {
                        ...
                        uint256 amountOut = DexLogicLib.swapExactInputMEVUnsafe(
                            debtToken, supplyToken, dataCache.poolFee, debtWithdrawed, dexRouter
                        );
            ...
            AaveLogicLib.supplyAave(
                supplyToken, deltaToSupply, address(this), aavePoolAddressesProvider
            );
        } else {
    ...
    function _bringingToTargetBorrowAmount(
        ...
        uint256 totalDebt = AaveLogicLib.getTotalDebt(address(debtTokenAave), address(this));
        ...
        if (amountToBorrow > totalDebt) {
            ...
            AaveLogicLib.borrowAave(debtToken, amount, address(this), aavePoolAddressesProvider);
        } else {
            ...
            if (isDeposit) {
                debtTokenBalance = TransferHelper.safeGetBalance(debtToken, address(this));
            } else {
                debtTokenBalance = 0;
            }

            dataCache.sqrtPriceX96 = DexLogicLib.getCurrentSqrtRatioX96(dexPool);
            ...
            if (debtTokenBalance < deltaToRepay) {
                if (dataCache.nftLiquidity != 0) {
                    ...
                    if (deltaToRepay > debtTokenBalance && supplyWithdrawed > 0) {
                        // swap supplyToken to debtToken
                        uint256 amountOut = DexLogicLib.swapExactInputMEVUnsafe(
                            supplyToken, debtToken, dataCache.poolFee, supplyWithdrawed, dexRouter
                        );
            ...
            if (deltaToRepay > debtTokenBalance) {
                ...
                if (debtToken < supplyToken) {
                    token1Token0Quote = DexLogicLib.getAmount0InToken1(
                        dataCache.sqrtPriceX96, deltaToRepay - debtTokenBalance
                    );
                } else {
                ...
                AaveLogicLib.withdrawAave(
                    supplyToken, token1Token0Quote, address(this), aavePoolAddressesProvider
                );

                supplyTokenBalance =
                    TransferHelper.safeGetBalance(supplyToken, address(this)) - supplyTokenBalance;

                // swap supplyToken to debtToken
                DexLogicLib.swapExactInputMEVUnsafe(
                    supplyToken, debtToken, dataCache.poolFee, supplyTokenBalance, dexRouter
                );
            }
            ...
            AaveLogicLib.repayAave(
                debtToken, deltaToRepay, address(this), aavePoolAddressesProvider
            );
        }
    }
    ...
    function _getTotalSupplyTokenBalance(
        ...
        uint256 liquidity = DexLogicLib.getLiquidity(nftId, dexNftPositionManager);
        if (liquidity != 0) {
            (amount0, amount1) = DexLogicLib.tvl(nftId, dexPool, dexNftPositionManager);
        ...
        uint256 pureAmountSupplyToken = TransferHelper.safeGetBalance(
            address(supplyTokenAave), address(this)
        ) + amount0 + amountToDeposit;
        ...
        if (debtToken < supplyToken) {
            pureAmountDebtTokenInSupplyToken = DexLogicLib.getAmount0InToken1(sqrtPriceX96, amount1);

            totalDebtInSupplyToken = DexLogicLib.getAmount0InToken1(sqrtPriceX96, totalDebt);
        } else {
            pureAmountDebtTokenInSupplyToken = DexLogicLib.getAmount1InToken0(sqrtPriceX96, amount1);

            totalDebtInSupplyToken = DexLogicLib.getAmount1InToken0(sqrtPriceX96, totalDebt);
        }

        // returns total supplyToken balance
        return pureAmountSupplyToken + pureAmountDebtTokenInSupplyToken - totalDebtInSupplyToken;
    }
    ...
    function _getAmountToSupplyAndToBorrow(
        ...
        // get ticks
        (,,, int24 minTick, int24 maxTick,) = DexLogicLib.getNftData(nftId, dexNftPositionManager);

        uint160 sqrtPriceX96 = DexLogicLib.getCurrentSqrtRatioX96(dexPool);

        uint256 currentHealthFactorForCalculation_1e18 =
            _getHFForCalculations(targetHealthFactor_e18, pointerToAaveChecker);

        // get current liquidation threshold
        uint256 currentLiquidationThreshold_1e4 =
            AaveLogicLib.getCurrentLiquidationThreshold(supplyToken, aavePoolAddressesProvider);

        uint256 R_e18 = DexLogicLib.getTargetRE18ForTickRange(
            minTick, maxTick, dexPool.liquidity(), sqrtPriceX96
        );

        amountToSupply = (currentHealthFactorForCalculation_1e18 * inputAmount * (E18 - R_e18))
            / (
                currentHealthFactorForCalculation_1e18 * (E18 - R_e18)
                    + R_e18 * currentLiquidationThreshold_1e4 * E14
            );
        ...
        if (supplyToken < debtToken) {
            amountToBorrow = DexLogicLib.getAmount0InToken1(sqrtPriceX96, borrowMultiplier);
        } else {
            amountToBorrow = DexLogicLib.getAmount1InToken0(sqrtPriceX96, borrowMultiplier);
        }
    }
    ...
    function _getHFForCalculations(
        ...
        uint256 currentHealthFactor =
            AaveLogicLib.getCurrentHF(address(this), aavePoolAddressesProvider);

        if (currentHealthFactor < lowerHFBoundary || currentHealthFactor > upperHFBoundary) {
    ...
    function _convertAssetsToSupplyToken(
        ...
        DexLogicLib.swapExactInputMEVUnsafe(
            debtToken, supplyToken, poolFee, debtTokenBalance, dexRouter
        );
    }
    ...
    function _depositToUni(
        ...
        if (supplyToken > debtToken) {
            (supplyTokenAmount, debtTokenAmount) = (debtTokenAmount, supplyTokenAmount);
            (supplyToken, debtToken) = (debtToken, supplyToken);
        }

        (supplyTokenAmount, debtTokenAmount) = DexLogicLib.swapToTargetRMEVUnsafe(
            tickLower,
            tickUpper,
            supplyTokenAmount,
            debtTokenAmount,
        ...
        TransferHelper.safeApprove(supplyToken, address(dexNftPositionManager), supplyTokenAmount);
        TransferHelper.safeApprove(debtToken, address(dexNftPositionManager), debtTokenAmount);

        DexLogicLib.increaseLiquidityMEVUnsafe(
            nftId, supplyTokenAmount, debtTokenAmount, dexNftPositionManager
        );
    }
    ...
    /// @param supplyToken The supply token address.
    /// @param debtToken The debt token address.
    function _validateTokens(uint256 nftId, address supplyToken, address debtToken) internal view {
        (address token0, address token1,,,,) = DexLogicLib.getNftData(nftId, dexNftPositionManager);
    ...
    function _validateAaveCheckerPointer(bytes32 aaveCheckerPointer) private view {
        (,,, bool initialized) =
            IAaveCheckerLogic(address(this)).getLocalAaveCheckerStorage(aaveCheckerPointer);
