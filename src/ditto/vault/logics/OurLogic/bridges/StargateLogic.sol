...
contract StargateLogic is IStargateLogic, BridgeLogicBase {
    ...
    function _getStargateStorage() private view returns (StargateLogicStorage storage s) {
    ...
    /// @notice Initializes the contract with the stargate composer and ditto stargate receiver
    /// addresses.
    ...
    function sendStargateMessage(
        ...
        (address owner, uint16 vaultId) =
            _validateBridgeCall(StargateLogic_VaultCannotUseCrossChainLogic.selector);

        bytes memory newPayload = abi.encode(owner, vaultVersion, vaultId, payload);

        (uint256 fee,) = stargateComposer.quoteLayerZeroFee(
            dstChainId, 1, abi.encodePacked(dittoReceiver), newPayload, lzTxParams
        );

        TransferHelper.safeApprove(
            IStargatePool(IStargateFactory(stargateComposer.factory()).getPool(srcPoolId)).token(),
    ...
    function stargateMultisender(
        ...
        (uint256 fee,) = stargateComposer.quoteLayerZeroFee(
            mParams.dstChainId, 1, abi.encodePacked(address(0)), bytes(""), lzTxParams
        );
        ...
        TransferHelper.safeApprove(
            IStargatePool(IStargateFactory(stargateComposer.factory()).getPool(mParams.srcPoolId))
                .token(),
            address(stargateComposer),
            bridgeAmount
        );
    ...
    /// @inheritdoc IStargateLogic
    function stargateMulticall(uint256 bridgedAmount, bytes[] calldata data) external {
