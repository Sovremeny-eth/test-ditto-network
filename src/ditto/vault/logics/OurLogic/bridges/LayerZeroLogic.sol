...
contract LayerZeroLogic is ILayerZeroLogic, BridgeLogicBase {
    ...
    /// @notice Initializes the contract with the layer zero endpoint and ditto layer zero receiver
    /// addresses.
    ...
    function sendLayerZeroMessage(
        ...
        (address owner, uint16 vaultId) =
            _validateBridgeCall(LayerZeroLogic_VaultCannotUseCrossChainLogic.selector);

        bytes memory newPayload = abi.encode(owner, vaultVersion, vaultId, payload);

        bytes memory adapterParam = _txParamBuilder(
            lzTxParams.dstGasForCall, lzTxParams.dstNativeAmount, lzTxParams.dstNativeAddr
        );

        (uint256 fee,) = layerZeroEndpoint.estimateFees(
            dstChainId, address(this), newPayload, lzTxParams.payInZRO, adapterParam
        );
    ...
    /// @param dstNativeAmount The amount of native token to be sent along with the call on the
    /// destination chain.
    ...
    function _txParamBuilder(
        ...
        if (dstNativeAmount > 0 && dstNativeAddr != address(0)) {
            ...
            adapterParam = abi.encodePacked(txType, dstGasForCall, dstNativeAmount, dstNativeAddr);
        } else {
            txType = 1;
