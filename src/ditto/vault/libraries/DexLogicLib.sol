...
library DexLogicLib {
    ...
    function getCurrentSqrtRatioX96(IUniswapV3Pool _dexPool)
        internal
        view
    ...
    function getLiquidity(
        ...
        (,,,,,,, uint128 liquidity,,,,) = dexNftPositionManager.positions(nftId);
        return liquidity;
    }

    /// @notice Calculates the amount of token0 in terms of token1 based on the square root of the
    /// price.
    ...
    /// @dev Calculates the amount of token1 in terms of token0 based on the square root of the
    /// price.
    /// @param sqrtPriceX96 The square root of the price, represented as a X96 fixed point number.
    ...
    function getTargetRE18ForTickRange(
        ...
        (uint256 amount0ForLiquidity, uint256 amount1ForLiquidity) = LiquidityAmounts
        ...
        uint256 amount0ForLiquidityInToken1 = getAmount0InToken1(sqrtPriceX96, amount0ForLiquidity);
    ...
    {
        (,, token0, token1, poolFee, tickLower, tickUpper, liquidity,,,,) =
    ...
    function MEVCheck(
        ...
        int24 timeWeightedAverageTick = UniswapOracleLibrary.consult(_dexPool, period);

        uint256 oraclePrice =
            getAmount0InToken1(TickMath.getSqrtRatioAtTick(timeWeightedAverageTick), E18);
    ...
    /// @param dexNftPositionManager The Nonfungible Position Manager from which the position is
    /// withdrawn.
    ...
    function swapToTargetRMEVUnsafe(
        ...
        uint160 sqrtPriceX96 = getCurrentSqrtRatioX96(_dexPool);
        uint256 targetRE18 =
            getTargetRE18ForTickRange(tickLower, tickUpper, _dexPool.liquidity(), sqrtPriceX96);

        uint256 amount1Target = token1AmountAfterSwapForTargetRE18(
            sqrtPriceX96, token0Amount, token1Amount, targetRE18, poolFee
        );
    ...
    ///
    /// @dev y1 = (R1 - Rtg) / (R1 - Rtg * poolFee / feeMax) * (y0 +  p * x0 * (F1 - poolFee) /
    /// feeMax)
    /// where:
    ...
    function swapAssetsMEVUnsafe(
        ...
        if (amount1 > amount1Target) {
            ...
            uint256 amountOut =
                swapExactInputMEVUnsafe(token1, token0, poolFee, amountForSwap, dexRouter);
            unchecked {
                // update balances
            ...
            if (targetR == 0) {
                ...
                uint256 amountOut =
                    swapExactInputMEVUnsafe(token0, token1, poolFee, amount0, dexRouter);
                unchecked {
                    // update balances
                ...
                // Since we don't know the exact number of tokens to be given in the SwapExactOutput
                // method,
                // we do approve for the entire transferred balance of token0
                TransferHelper.safeApprove(token0, address(dexRouter), amount0);

                uint256 amountIn =
                    swapExactOutputMEVUnsafe(token0, token1, poolFee, amountForSwap, dexRouter);
    ...
    function swapExactOutputMEVUnsafe(
        ...
        return dexRouter.exactOutputSingle(
    ...
    function mintNftMEVUnsafe(
        ...
        TransferHelper.safeApprove(token0, address(dexNftPositionManager), token0Amount);
        TransferHelper.safeApprove(token1, address(dexNftPositionManager), token1Amount);
    ...
    function validateTokenBalance(address token, uint256 tokenAmount) internal view {
        uint256 tokenBalance = TransferHelper.safeGetBalance(token, address(this));
    ...
    function fees(
        ...
        (uint256 amount0, uint256 amount1) =
            PositionValueMod.fees(dexNftPositionManager, nftId, _dexPool);
        return (amount0, amount1);
    }
    ...
    function tvl(
        ...
        (uint256 amount0, uint256 amount1) =
            PositionValueMod.total(dexNftPositionManager, nftId, sqrtPriceX96, _dexPool);
        return (amount0, amount1);
    }
    ...
    function principal(
        ...
        (uint256 amount0, uint256 amount1) =
            PositionValueMod.principal(dexNftPositionManager, nftId, sqrtPriceX96);
        return (amount0, amount1);
    }
}
