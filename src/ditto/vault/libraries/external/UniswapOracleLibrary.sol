...
library UniswapOracleLibrary {
    ...
    /// @param secondsAgo Number of seconds in the past from which to calculate the time-weighted
    /// means
    /// @return arithmeticMeanTick The arithmetic mean tick from (block.timestamp - secondsAgo) to
    /// block.timestamp or block starting tick
    function consult(
        IUniswapV3Pool pool,
        uint32 secondsAgo
        ...
        try pool.observe(secondsAgos) returns (
            ...
            arithmeticMeanTick = _calculateArithmeticMeanTick(tickCumulatives, secondsAgo);
        } catch {
...

            {
                ...
                (, arithmeticMeanTick, observationIndex, observationCardinality,,,) =
            ...
            if (observationCardinality == 1) {
                ...
                (uint32 observationTimestamp, int56 tickCumulative,,) =
                ...
                unchecked {
                    ...
                    arithmeticMeanTick = int24(
                        (tickCumulative - int56(uint56(prevTickCumulative))) / int56(uint56(delta))
                    );
                }
            }
        }
    ...
    /// means
    /// @return arithmeticMeanTick The arithmetic mean tick from (block.timestamp - secondsAgo) to
    /// block.timestamp or current tick
