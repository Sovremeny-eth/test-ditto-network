...
library AaveLogicLib {
    ...
    function emergencyRepayAave(
        ...
        pool.flashLoanSimple(
            address(this), debtToken, getTotalDebt(aDebtToken, onBehalfOf), params, REFFERAL_CODE
        );

        // return remaining supply amount back to aave
        remainingSupply = IERC20(supplyToken).balanceOf(address(this)) - remainingSupply;
    ...
    function executeOperation(
        ...
        (address supplyToken, address onBehalfOf, uint24 poolFee) =
        ...
        if (onBehalfOf != address(this)) {
            ...
            IAToken(aSupplyToken).transferFrom(onBehalfOf, address(this), supplyAmount);
        }
    ...
    function getSupplyAmount(address supplyToken, address user) internal view returns (uint256) {
    ...
    function getTotalDebt(address debtToken, address user) internal view returns (uint256) {
    ...
    function getCurrentHF(
        ...
        (,,,,, currentHF) = IPool(poolAddressesProvider.getPool()).getUserAccountData(user);
    }
    ...
    function getCurrentLiquidationThreshold(
        ...
        (,, currentLiquidationThreshold_1e4,,,,,,,) =
            poolDataProvider.getReserveConfigurationData(token);
    }
    ...
    function aDebtTokenAddress(address asset, IPool pool) internal view returns (address) {
    ...
    function aSupplyTokenAddress(address asset, IPool pool) internal view returns (address) {
