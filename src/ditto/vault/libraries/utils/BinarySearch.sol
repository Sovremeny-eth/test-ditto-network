...
library BinarySearch {
    ...
    /// by `selectors`. If the function `selector` exists, the associated `logic` address is
    /// returned.
    /// @param selector The function selector (4 bytes) to search for.
    /// @param logicsAndSelectors The concatenated bytes array of logic addresses and function
    /// selectors.
    /// @return logic The logic address associated with the given function selector, or address(0)
    /// if not found.
