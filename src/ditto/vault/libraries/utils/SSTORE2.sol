...
library SSTORE2 {
    uint256 internal constant DATA_OFFSET = 1; // We skip the first byte as it's a STOP opcode to
        // ensure the contract can't be called.
    ...
    function write(bytes memory data) internal returns (address pointer) {
        bytes memory creationCode = bytes.concat(
            ...
            // 0x80    |  0x80               | DUP          | (codeSize - codeOffset) (codeSize -
            // codeOffset) 0 codeOffset   //
            // 0x92    |  0x92               | SWAP3        | codeOffset (codeSize - codeOffset) 0
            // (codeSize - codeOffset)   //
            // 0x59    |  0x59               | MSIZE        | 0 codeOffset (codeSize - codeOffset) 0
            // (codeSize - codeOffset) //
            ...
            hex"00",
            // The bytecode we want the contract to have after deployment. Capped at 1 byte less
            // than the code size limit.
            data
        );
