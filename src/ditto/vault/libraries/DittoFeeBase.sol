...
contract DittoFeeBase {
    ...
    function _transferDittoFee(uint256 dittoFee, uint256 rollupFee, bool isInstant) internal {
        address treasury;
        uint256 feeGasBps;
        uint256 feeFix;

        if (isInstant) {
            (treasury, feeGasBps, feeFix) = _protocolFees.getInstantFeesAndTreasury();
        } else {
            (treasury, feeGasBps, feeFix) = _protocolFees.getAutomationFeesAndTreasury();
        }
        ...
        if (treasury != address(0)) {
            unchecked {
                ...
                dittoFee = (((dittoFee + rollupFee) * feeGasBps) / E18) + feeFix;
            }
