...
library AccessControlLib {
    ...
    /// @dev If this smart-contract is an owner of other contract - this contract must implement
    /// ERC1271 method.
    ...
    function isValidSignature(
        ...
        if (retAddress != owner) {
            ...
            retAddress = _dataHash.recover(signature);
        }
        ...
        if (!isValidSig) {
            ...
            assembly ("memory-safe") {
                ...
                // pop - the reason for failure does not matter, validation is performed in another
                // way
                pop(staticcall(gas(), owner, add(ptr, 28), add(100, sigLen), 0, 0))

                returndatacopy(0, 0, returndatasize())

                // if returndatasize < 32 or returndatasize > 32 -> return empty value
                if or(lt(returndatasize(), 32), gt(returndatasize(), 32)) { mstore(0, 0) }
            ...
            if (eip1271Value != EIP1271_MAGIC_VALUE) {
                ...
                eip1271Value = IERC1271(owner).isValidSignature(dataHash, signature);
            }
