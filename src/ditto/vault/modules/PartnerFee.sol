...
contract PartnerFee is IPartnerFee, Ownable {
    ...
    function withdrawReward(address _token) external {
        ...
        if (balance > 0) {
            ...
            if (rewardPartner > 0) {
                IERC20Metadata(_token).transfer(_partnerReceiver, rewardPartner);
            }
        ...
        returns (address[] memory tokens, uint256[] memory fees)
    {
        ...
        mapping(address => uint256) storage feesByToken = _currentAgreement.feesByToken;
    ...
    function getFixFeesForPartnerByToken(address tokenAddress)
        external
        view
    ...
    function _setAgreement(
        ...
        mapping(address => uint256) storage feesByToken = _currentAgreement.feesByToken;
        mapping(address => bool) storage allowedTokens = _currentAgreement.allowedTokens;
        ...
        emit AgreementUpdated(
            dittoController, partnerController, dittoReceiver, partnerReceiver, partnerPortionBPS
        );
    }
