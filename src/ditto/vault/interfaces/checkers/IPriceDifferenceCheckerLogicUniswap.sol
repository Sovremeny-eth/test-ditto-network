...
interface IPriceDifferenceCheckerLogicUniswap is IPriceDifferenceCheckerLogicBase {
    // =========================
    // Initializer
    // =========================
    ...
    function uniswapCheckPriceDifference(bytes32 pointer) external returns (bool success);

    /// @notice Checks the percentage difference between the current price and the last checked
    /// price.
    /// @param pointer The bytes32 pointer value.
    /// @return success True if the percentage difference is within an acceptable range.
    function uniswapCheckPriceDifferenceView(bytes32 pointer)
        external
        view
    ...
    function uniswapGetLocalPriceDifferenceCheckerStorage(bytes32 pointer)
        external
        view
        returns (
            address token0,
