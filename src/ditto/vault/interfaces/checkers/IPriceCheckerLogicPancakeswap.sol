...
interface IPriceCheckerLogicPancakeswap is IPriceCheckerLogicBase {
    ...
    function pancakeswapCheckGTTargetRate(bytes32 pointer) external view returns (bool);

    /// @notice Checks if the current rate is greater than or equal to the target rate.
    /// @param pointer The bytes32 pointer value.
    /// @return bool indicating whether the current rate is greater than or equal to the target
    /// rate.
    function pancakeswapCheckGTETargetRate(bytes32 pointer) external view returns (bool);
    ...
    function pancakeswapCheckLTTargetRate(bytes32 pointer) external view returns (bool);
    ...
    function pancakeswapCheckLTETargetRate(bytes32 pointer) external view returns (bool);
    ...
    /// @param pointer The bytes32 pointer value.
    function pancakeswapChangeTargetRate(uint256 targetRate, bytes32 pointer) external;
    ...
    function pancakeswapGetLocalPriceCheckerStorage(bytes32 pointer)
        external
        view
        returns (address token0, address token1, uint24 fee, uint256 targetRate, bool initialized);
}
