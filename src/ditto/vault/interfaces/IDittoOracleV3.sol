...
interface IDittoOracleV3 {
    ...
    /// @param dexFactory Factory address of the UniswapV3-like protocol. (e.g.: Pancakeswap,
    /// Uniswap, etc.)
    /// @return amountOut The amount of tokens that the pool can approximately exhange for
    /// `amountIn`.
    /// @dev If pool with tokens and fee does not exist in the protocol,
    /// `UniswapOracle_PoolNotFound` error is thrown.
    function consult(
        address tokenIn,
        uint256 amountIn,
        address tokenOut,
