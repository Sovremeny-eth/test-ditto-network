...
contract Vault is IVault, BaseContract {
    ...
    /// @dev Mapping that stores module addresses according to the selector that belongs to that
    /// module.
    mapping(bytes4 functionSelector => address logicAddress) private _moduleAddress;
    ...
    /// @param protocolModuleList Address that stores the list of module addresses and their
    /// selectors.
    ///
    ...
    constructor(
        ...
        assembly ("memory-safe") {
            ...
            for { } lt(i, selectorsLength) {
                ...
                logicAndSelectorValue :=
                    or(mload(selectorsOffset), shl(64, mload(logicsAddressesOffset)))
    ...
    /// @inheritdoc IVault
    function moduleAction(address moduleAddress, ActionModule action) external onlyVaultItself {
    ...
    function _getAddress(bytes4 selector) internal view returns (address logic) {
        bytes memory logicsAndSelectors = SSTORE2.read(logicsAndSelectorsAddress);
    ...
    function _addModule(address moduleAddress) internal {
        (bytes memory selectors, bool inactive) =
            _protocolModuleList.getSelectorsByModule(moduleAddress);
    ...
    function _deleteModule(address moduleAddress) internal {
        ...
        (bytes memory selectors,) = _protocolModuleList.getSelectorsByModule(moduleAddress);
