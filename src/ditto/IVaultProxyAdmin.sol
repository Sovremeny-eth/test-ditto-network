...
interface IVaultProxyAdmin {
    ...
    /// @notice Thrown when an anyone other than the address of the factory tries calling the
    /// method.
    ...
    function initializeImplementation(address vault, address implementation) external;

    /// @notice Updates the `vault` implementation to an address from the factory.
    /// @param vault Address of the vault to be upgraded.
    /// @param version The version number of the new implementation from the `_implementations`
    /// array.
    ///
