...
/// @dev This contract interacts with Aave V3 and provides helper methods for fetching user-specific
/// data
contract AaveLogicLens is IAaveLogicLens {
    ...
    function getSupplyAmount(address supplyToken, address user) external view returns (uint256) {
        address aSupplyToken =
            AaveLogicLib.aSupplyTokenAddress(supplyToken, IPool(poolAddressesProvider.getPool()));
        return AaveLogicLib.getSupplyAmount(aSupplyToken, user);
    }

    /// @inheritdoc IAaveLogicLens
    function getTotalDebt(address debtToken, address user) external view returns (uint256) {
        address aDebtToken =
            AaveLogicLib.aDebtTokenAddress(debtToken, IPool(poolAddressesProvider.getPool()));
        return AaveLogicLib.getTotalDebt(aDebtToken, user);
    }

    /// @inheritdoc IAaveLogicLens
    function getCurrentHF(address user) external view returns (uint256 currentHF) {
        return AaveLogicLib.getCurrentHF(user, poolAddressesProvider);
    }

    /// @inheritdoc IAaveLogicLens
    function getCurrentLiquidationThreshold(address token)
        external
        view
        returns (uint256 currentLiquidationThreshold_1e4)
    {
        return AaveLogicLib.getCurrentLiquidationThreshold(token, poolAddressesProvider);
    }
}
