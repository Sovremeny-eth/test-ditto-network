...
/// @notice Kernel validator used to validated user operations via WebAuthn signature (using P256
/// under the hood)
...
contract WebAuthnFclValidator is IKernelValidator {
    /// @dev Event emitted when the public key signing the WebAuthN user operation is changed for a
    /// given `kernel`.
    /// @dev The `b64AuthenticatorId` param represent the webauthn authenticator id used to create
    /// this public key
    event WebAuthnPublicKeyChanged(
        address indexed kernel, string indexed b64AuthenticatorId, uint256 x, uint256 y
    );

    /// @dev Mapping of kernel address to each webAuthn specific storage
    mapping(address kernel => WebAuthnFclValidatorStorage webAuthnStorage) private
        webAuthnValidatorStorage;

    /// @dev The address of the on-chain p256 verifier contract (will be used if the user want that
    /// instead of the pre-compiled one, that way this validator can work on every chain out of the
    /// box while rip7212 is slowly being implemented everywhere)
    ...
    function enable(bytes calldata _data) external payable override {
        ...
        (string memory authenticatorId, uint256 x, uint256 y) =
            abi.decode(_data, (string, uint256, uint256));
        // Update the pub key data
        WebAuthnFclValidatorStorage storage kernelValidatorStorage =
            webAuthnValidatorStorage[msg.sender];
    ...
    /// @dev Validate a `_userOp` using a WebAuthn Signature for the kernel account who is the
    /// `_userOp` sender
    function validateUserOp(
        ...
        WebAuthnFclValidatorStorage memory kernelValidatorStorage =
            webAuthnValidatorStorage[_userOp.sender];
    ...
    /// (msg.sender)
    /// @param _kernelValidatorStorage The kernel storage replication (helping us to fetch the X & Y
    ...
    /// @dev Check if the caller is a valid signer, this don't apply to the WebAuthN validator,
    /// since it's using a public key
    ...
    function getPublicKey(address _kernel) public view returns (uint256 x, uint256 y) {
        ...
        WebAuthnFclValidatorStorage storage kernelValidatorStorage =
            webAuthnValidatorStorage[_kernel];
    ...
    function isPreCompiledP256Available() public view returns (bool) {
        ...
        (bool success, bytes memory data) =
            WebAuthnFclVerifier.PRECOMPILED_P256_VERIFIER.staticcall(testSignatureData);
