...
import { ValidationData, ValidAfter, ValidUntil } from "src/kernel/common/Types.sol";
...
import { _intersectValidationData } from "src/kernel/utils/KernelHelper.sol";
...
contract ModularPermissionValidator is IKernelValidator {
    ...
    mapping(
        bytes32 permissionId
            => mapping(PolicyConfig policy => mapping(address kernel => PolicyConfig))
    ) public nextPolicy;
    ...
    ) external view override returns (ValidationData validationData) {
