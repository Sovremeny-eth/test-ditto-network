...
import {
    ValidationData, ValidUntil, ValidAfter, packValidationData
} from "src/kernel/common/Types.sol";
import { SIG_VALIDATION_FAILED } from "src/kernel/common/Constants.sol";
...
contract GasPolicy is IPolicy {
    ...
    mapping(
        address permissionValidator
            => mapping(bytes32 permissionId => mapping(address kernel => GasPolicyConfig))
    ...
    function registerPolicy(
        ...
        (uint128 allowed, bool enforcePaymaster, address allowedPaymaster) =
            abi.decode(data, (uint128, bool, address));
        gasPolicyConfig[msg.sender][permissionId][kernel] =
            GasPolicyConfig(allowed, enforcePaymaster, allowedPaymaster);
    }
