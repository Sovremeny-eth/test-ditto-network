...
/// From
/// https://github.com/cometh-hq/p256-signer/blob/09319213276da69aad6d96fa75cd339726f78bb9/contracts/P256Signer.sol
...
library WebAuthnFclVerifier {
    ...
    /// @dev Always 0x01 for user presence flag ->
    /// https://www.w3.org/TR/webauthn-2/#concept-user-present
    ...
    /// @dev Logic from
    /// https://github.com/rdubois-crypto/FreshCryptoLib/blob/master/solidity/src/FCL_Webauthn.sol
    ...
    function _formatWebAuthNChallenge(
        bytes32 _hash,
        FclSignatureLayout calldata _signature
    ) internal pure returns (bytes32 p256Message) {
        ...
        // If the challenge offset is uint256 max, it's mean that we are in the case of a dummy sig,
        // so we can skip the check and just return the hash
        ...
        // Otherwise, perform the complete format and checks of the data
        {
            ...
            if (
                (authenticatorData[32] & AUTHENTICATOR_DATA_FLAG_MASK)
                    != AUTHENTICATOR_DATA_FLAG_MASK
            ) {
                revert InvalidWebAuthNData();
            }
            // Verify that clientData commits to the expected client challenge
            // Use the Base64Url encoding which omits padding characters to match WebAuthn
            // Specification
            bytes memory challengeEncoded =
                bytes(Base64.encode(abi.encodePacked(_hash), true, true));
            ...
            assembly {
                ...
                // Check that the challenge extracted from the clientData is the same as the one
                // provided in the userOp
                if iszero(
                    eq(
                        // Hash of the challenge exracted from the `clientData`
                        keccak256(add(challengeExtracted, 32), mload(challengeExtracted)),
                        // Hash of the provided challenge, encoded in Base64Url (to match the
                        // clientData encoding)
    ...
    /// @param _p256Verifier The p256 verifier contract on-chain (if user want to use this instead
