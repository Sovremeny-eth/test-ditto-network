...
contract ECDSASigner is ISigner {
    ...
    mapping(address caller => mapping(bytes32 permissionId => mapping(address kernel => address)))
        public signer;
    ...
    function registerSigner(
        ...
        require(
            signer[msg.sender][permissionId][kernel] == address(0),
            "ECDSASigner: kernel already registered"
        );
    ...
    function validateUserOp(
        ...
        require(
            signer[msg.sender][permissionId][kernel] != address(0),
            "ECDSASigner: kernel not registered"
        );
