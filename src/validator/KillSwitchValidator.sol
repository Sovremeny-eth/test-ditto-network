...
contract KillSwitchValidator is IKernelValidator {
    ...
    function validateUserOp(
        ...
        KillSwitchValidatorStorage storage validatorStorage = killSwitchValidatorStorage[msg.sender]; // should
            // use msg.sender to prevent others from changing storage
        ...
        if (address(validatorStorage.validator) != address(0)) {
            ...
            try validatorStorage.validator.validateUserOp(_userOp, _userOpHash, 0) returns (
                ValidationData res
            ) {
                validationData = res;
            } catch {
        ...
        if (_userOp.signature.length == 71) {
            ...
            bytes32 hash = ECDSA.toEthSignedMessageHash(
                keccak256(bytes.concat(_userOp.signature[0:6], _userOpHash))
            );
